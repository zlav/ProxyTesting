...
echo '' > response-code-ok.txt

header="$HEADER: $headerval"

echo $header > response-code-ok.txt

echo "Loop URL $URL"
for i in `seq 1 $ITERATIONS`;
   do
         sleep .2
	  curl -sL -w "%{http_code}" -I $URL -o /dev/null >> response-code-ok.txt 2>&1
	  echo $'\n' >> reponse-code-ok.txt 2>&1 
   done

echo "Done - check File response-code-ok.txt for Results" 
