...
echo '' > response-code-ok-2.txt

header="$HEADER: $headerval"

echo $header > response-code-ok-2.txt

echo "Loop URL $URL"
for i in `seq 1 $ITERATIONS`;
   do
         # sleep .01
	  curl -sL -w "%{http_code}" -I $URL -o /dev/null >> response-code-ok-2.txt 2>&1
	  echo $'\n' >> reponse-code-ok-2.txt 2>&1 
   done

echo "Done - check File response-code-ok-2.txt for Results" 
