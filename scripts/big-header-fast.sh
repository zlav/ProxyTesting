...
echo '' > response-code-fast.txt

header="$HEADER: $headerval"

echo $header > response-code-fast.txt

echo "Loop URL $URL"
for i in `seq 1 $ITERATIONS`;
   do
         sleep 1
	 curl -i -H 'Authorization: Basic hi there' $URL  >> response-code-fast.txt 2>&1
   done

echo "Done - check File response-code-fast.txt for Results" 
